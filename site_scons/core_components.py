...
class UnitTestComponent(ProgramComponent):
    ...
    def __init__(self, graph, env, name, dir, deps, inc, src, alias=None):
        ProgramComponent.__init__(self,graph,env,name,dir,deps,inc,src,alias)
    ...
    def _CreateJenkinsTarget(self):
        ...
        self.jenkins_target = self._env.Alias('%s:jenkins' % name, None, '')
        self._env.AlwaysBuild(self.jenkins_target)

    def _CreateReadyToCommitTtarget(self):
        self.ready_to_commit = self._env.RunReadyToCommit(self.name, None)
        self._env.Alias('%s:ready-to-commit' % self.name.split(':')[0], self.ready_to_commit, '')
        self._env.AlwaysBuild(self.ready_to_commit)
    ...
    #def GetIncludePaths(self):
        ...
        #filtered_includes = [self._dir.abspath, project.dir]
        ...
        #prjInstallPath = os.path.join(self._env['INSTALL_HEADERS_DIR'], project.name)
        #for i in includes:
            #path = os.path.realpath(i)
            #in_build_dir = self._env['BUILD_DIR'] in path
